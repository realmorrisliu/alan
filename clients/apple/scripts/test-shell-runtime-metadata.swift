...
private enum ShellRuntimeMetadataTests {
    static func run() {
        ...
        verifiesRuntimeProjectsTerminalGridDiagnosticsIntoPaneContext()
        verifiesTerminalTranscriptDimensionsFollowAuthoritativeGrid()
        verifiesTerminalGridDiagnosticsSerializeUnavailableAndMismatch()
        verifiesPointOnlyTerminalFrameChangeStaysDiagnostic()
        verifiesMaxFitGridDiagnosticsAcrossRepresentativeDisplayClasses()
        verifiesSidebarWidthGridRefreshPreservesSplitIdentity()
        ...
        verifiesExplicitTerminalTabTitleIsLockedAgainstRuntimeMetadata()
        ...
        verifiesControlPlaneGridPolicyGuardForMixedContentSplits()
    ...
    private static func verifiesRuntimeProjectsTerminalGridDiagnosticsIntoPaneContext() {
        let controller = makeController()
        guard let pane = controller.selectedPane else {
            fail("bootstrap shell must expose a selected pane")
        }

        let diagnostics = TerminalGridDiagnostics(
            plannedGrid: TerminalGridDimensions(columns: 145, rows: 41),
            rendererGrid: TerminalGridDimensions(columns: 145, rows: 41),
            ptyGrid: TerminalGridDimensions(columns: 145, rows: 41),
            canvasPoints: TerminalGridPointSize(width: 1240, height: 760),
            backingPoints: TerminalGridPointSize(width: 2480, height: 1520),
            cellPoints: TerminalGridPointSize(width: 8.55, height: 18.5),
            layoutPolicy: .maxFit,
            remainder: TerminalGridRemainder(widthPoints: 0.25, heightPoints: 1.5),
        ...
        controller.updateTerminalRuntime(
            TerminalHostRuntimeSnapshot(
                ...
                paneID: pane.paneID,
                tabID: pane.tabID,
                logicalSize: CGSize(width: 1240, height: 760),
                backingSize: CGSize(width: 2480, height: 1520),
                displayName: "Studio Display",
                displayID: "display_1",
                attachedWindowTitle: "alan",
                isFocused: true,
                ...
                surfaceState: AlanTerminalSurfaceStateSnapshot(
                    ...
                    childExited: false,
                    terminalGridDiagnostics: diagnostics,
                    lastUpdatedAt: Date(timeIntervalSince1970: 1_011)
                ),
                terminalGridDiagnostics: diagnostics,
                lastUpdatedAt: Date(timeIntervalSince1970: 1_012)
            )
        )

        let updated = controller.shellState.panes.first { $0.paneID == pane.paneID }
        let projected = updated?.context?.terminalGridDiagnostics
        expect(projected?.plannedGrid?.columns == 145, "pane context must expose planned terminal columns")
        expect(projected?.plannedGrid?.rows == 41, "pane context must expose planned terminal rows")
        expect(projected?.canvasPoints?.width == 1240, "pane context must keep canvas points diagnostic")
        expect(projected?.mismatchStatus == .converged, "pane context must expose grid mismatch status")
    }

    private static func verifiesTerminalTranscriptDimensionsFollowAuthoritativeGrid() {
        let controller = makeController()
        guard let pane = controller.selectedPane else {
            fail("bootstrap shell must expose a selected pane")
        }
        ...
        let diagnostics = TerminalGridDiagnostics(
            ...
            cellPoints: TerminalGridPointSize(width: 8.55, height: 18.5),
            layoutPolicy: .maxFit,
            remainder: TerminalGridRemainder(widthPoints: 14, heightPoints: 20),
            mismatchStatus: .converged
        )
        controller.updateTerminalRuntime(
            TerminalHostRuntimeSnapshot(
                ...
                surfaceState: AlanTerminalSurfaceStateSnapshot(
                    ...
                    childExited: false,
                    terminalGridDiagnostics: diagnostics,
                    lastUpdatedAt: Date(timeIntervalSince1970: 1_013)
                ),
                terminalGridDiagnostics: diagnostics,
                lastUpdatedAt: Date(timeIntervalSince1970: 1_014)
            )
        )

        let baselineContentState = controller.shellState.contentStateProjection()
        guard let originalSlot = baselineContentState.paneSlot(paneSlotID: pane.paneID),
              let originalContent = baselineContentState.content(contentID: originalSlot.contentID),
              let updatedPane = controller.shellState.pane(paneID: pane.paneID),
              let projectedPayload = ShellContentInstance
                .projectingTerminalPane(updatedPane, contentID: "content_terminal_grid_transcript")
                .payload
                .terminal
        else {
            fail("terminal transcript dimension setup must expose projected terminal content")
        }

        let staleTranscript = TerminalTranscriptSnapshot(
            contentID: "content_terminal_grid_transcript",
            cwd: "/repo",
            title: "Shell",
            dimensions: TerminalTranscriptDimensions(columns: 1240, rows: 760),
            viewport: TerminalTranscriptViewport(firstVisibleRow: 0, cursorRow: 0),
            transcriptLines: ["ready"],
            processSummary: nil,
            capturedAt: Date(timeIntervalSince1970: 1_015),
        ...
        let explicitSlot = ShellPaneSlot(
            ...
            contentID: staleTranscript.contentID,
            attention: originalSlot.attention
        )
        let explicitContent = ShellContentInstance(
            contentID: staleTranscript.contentID,
            kind: .terminal,
            title: projectedPayload.title ?? originalContent.title,
            iconName: originalContent.iconName,
            capabilities: originalContent.capabilities,
            payload: .terminal(
                ShellTerminalContentPayload(
                    launchTarget: projectedPayload.launchTarget,
                    cwd: projectedPayload.cwd,
                    title: projectedPayload.title,
                    transcriptSnapshot: staleTranscript,
                    terminalProfileID: projectedPayload.terminalProfileID,
                    terminalGridDiagnostics: projectedPayload.terminalGridDiagnostics
                )
            ),
            lifecycle: originalContent.lifecycle,
            rendererState: originalContent.rendererState
        )
        ...
        guard let shellState = explicitContentState.materializingShellState() else {
            fail("explicit terminal transcript state must materialize")
        }

        let refreshedPayload = shellState
            .contentStateProjection()
            .content(contentID: staleTranscript.contentID)?
            .payload
            .terminal
        expect(
            refreshedPayload?.transcriptSnapshot?.dimensions?.columns == 120,
            "transcript snapshot columns must follow the authoritative renderer grid"
        )
        expect(
            refreshedPayload?.transcriptSnapshot?.dimensions?.rows == 40,
            "transcript snapshot rows must follow the authoritative renderer grid"
        )
    }

    private static func verifiesTerminalGridDiagnosticsSerializeUnavailableAndMismatch() {
        let unavailableContext = ShellContextSnapshot(
        ...
        let unavailableDiagnostics = encodedTerminalGridDiagnostics(unavailableContext)
        expect(
            unavailableDiagnostics?["planned_grid"] is NSNull,
            "unavailable diagnostics must encode missing planned grid explicitly"
        )
        expect(
            unavailableDiagnostics?["mismatch_status"] as? String == "unavailable",
            "unavailable diagnostics must encode stable mismatch status"
        )
        let unavailableCanvas = unavailableDiagnostics?["canvas_points"] as? [String: Any]
        expect(
            jsonDouble(unavailableCanvas?["width"]) == 1240,
            "canvas point width must stay in canvas_points"
        )

        let mismatchContext = ShellContextSnapshot(
            workingDirectoryName: "alan",
            repositoryRoot: nil,
            gitBranch: nil,
            controlPath: nil,
            ...
            terminalGridDiagnostics: TerminalGridDiagnostics(
                plannedGrid: TerminalGridDimensions(columns: 145, rows: 41),
                rendererGrid: TerminalGridDimensions(columns: 144, rows: 41),
                ptyGrid: TerminalGridDimensions(columns: 145, rows: 41),
                canvasPoints: TerminalGridPointSize(width: 1240, height: 760),
                backingPoints: TerminalGridPointSize(width: 2480, height: 1520),
                cellPoints: TerminalGridPointSize(width: 8.55, height: 18.5),
        ...
        let mismatchDiagnostics = encodedTerminalGridDiagnostics(mismatchContext)
        let plannedGrid = mismatchDiagnostics?["planned_grid"] as? [String: Any]
        let rendererGrid = mismatchDiagnostics?["renderer_grid"] as? [String: Any]
        let mismatchCanvas = mismatchDiagnostics?["canvas_points"] as? [String: Any]
        expect(
            jsonInt(plannedGrid?["columns"]) == 145,
            "planned terminal columns must encode separately from point width"
        )
        expect(
            jsonInt(rendererGrid?["columns"]) == 144,
            "renderer terminal columns must encode separately from planned columns"
        )
        expect(
            jsonDouble(mismatchCanvas?["width"]) == 1240,
            "host point width must stay in canvas_points for mismatch diagnostics"
        )
        expect(
            mismatchDiagnostics?["mismatch_status"] as? String == "mismatch",
            "mismatch diagnostics must encode stable mismatch status"
        )
    }

    private static func verifiesPointOnlyTerminalFrameChangeStaysDiagnostic() {
        let controller = makeController()
        guard let pane = controller.selectedPane else {
            fail("bootstrap shell must expose a selected pane")
        }
        projectTerminalGridDiagnostics(
        ...
        let projected = controller.shellState.pane(paneID: pane.paneID)?
            .context?
            .terminalGridDiagnostics
        expect(
            projected?.plannedGrid?.columns == 145,
            "point-only frame changes must not become terminal column changes"
        )
        expect(
            projected?.canvasPoints?.width == 1241,
            "point-only frame changes must remain visible as canvas diagnostics"
        )
    }

    private static func verifiesMaxFitGridDiagnosticsAcrossRepresentativeDisplayClasses() {
        struct DisplayClassCase {
            let name: String
            let columns: Int
            let rows: Int
        ...
        let cases = [
            ...
            DisplayClassCase(
                name: "current built-in full visible frame",
                columns: 145,
                rows: 41,
                canvasWidth: 1240,
                canvasHeight: 760,
            ...
            DisplayClassCase(
                name: "ordinary Alan placed window",
                columns: 130,
                rows: 40,
                canvasWidth: 1113,
                canvasHeight: 748,
            ...
            DisplayClassCase(
                name: "accessibility cell metrics",
                columns: 96,
                rows: 28,
                canvasWidth: 1200,
                canvasHeight: 700,
        ...
        for displayCase in cases {
            let controller = makeController()
            guard let pane = controller.selectedPane else {
                fail("\(displayCase.name) setup must expose a selected pane")
            }
            projectTerminalGridDiagnostics(
                controller: controller,
                paneID: pane.paneID,
                columns: displayCase.columns,
                rows: displayCase.rows,
                canvasWidth: displayCase.canvasWidth,
                canvasHeight: displayCase.canvasHeight,
                cellWidth: displayCase.cellWidth,
                cellHeight: displayCase.cellHeight,
                policy: .maxFit
            )
            ...
            let projected = controller.shellState.pane(paneID: pane.paneID)?
                ...
                .terminalGridDiagnostics
            expect(
                projected?.layoutPolicy == .maxFit,
                "\(displayCase.name) must keep max-fit as the runtime grid policy"
            )
            expect(
                projected?.rendererGrid?.columns == displayCase.columns
                    && projected?.rendererGrid?.rows == displayCase.rows,
                "\(displayCase.name) must publish Ghostty renderer grid dimensions"
            )
            expect(
                projected?.ptyGrid == projected?.rendererGrid,
                "\(displayCase.name) must keep PTY grid aligned with renderer grid"
            )
            expect(
                projected?.canvasPoints?.width == displayCase.canvasWidth,
                "\(displayCase.name) must keep point width as canvas diagnostics"
            )
            expect(
                projected?.cellPoints?.width == displayCase.cellWidth,
                "\(displayCase.name) must preserve cell metrics as diagnostics"
            )
        }
    }

    private static func verifiesSidebarWidthGridRefreshPreservesSplitIdentity() {
        let controller = makeController()
        guard let secondPaneID = controller.splitPane(paneID: "pane_1", placement: .right),
              let tabID = controller.selectedTabID
        else {
            fail("sidebar grid refresh setup must create a split terminal tab")
        }
        guard let tab = controller.shellState.contentStateProjection().tab(tabID: tabID) else {
            fail("sidebar grid refresh setup must expose the selected tab")
        }

        let baselinePaneSlotIDs = tab.paneTree.paneSlotIDs
        let baselineContentIDs = baselinePaneSlotIDs.compactMap {
            controller.shellState.contentStateProjection().contentMounted(in: $0)?.contentID
        }
        let baselineSplitRatio = tab.paneTree.restoringPaneTree().splitNodes.first?.splitRatio
        ...
        guard let refreshedTab = controller.shellState.contentStateProjection().tab(tabID: tabID) else {
            fail("sidebar grid refresh must preserve the selected tab")
        }
        let refreshedPaneSlotIDs = refreshedTab.paneTree.paneSlotIDs
        let refreshedContentIDs = refreshedPaneSlotIDs.compactMap {
            controller.shellState.contentStateProjection().contentMounted(in: $0)?.contentID
        }
        expect(
            refreshedPaneSlotIDs == baselinePaneSlotIDs,
            "sidebar width grid refresh must preserve PaneSlot identities"
        )
        expect(
            refreshedContentIDs == baselineContentIDs,
            "sidebar width grid refresh must preserve terminal ContentInstance identities"
        )
        expect(
            refreshedTab.paneTree.restoringPaneTree().splitNodes.first?.splitRatio == baselineSplitRatio,
            "sidebar width grid refresh must preserve split ratio"
        )
        expect(
            controller.shellState.pane(paneID: "pane_1")?
                .context?
                .terminalGridDiagnostics?
                .rendererGrid?
                .columns == 130,
            "sidebar width grid refresh must publish the refreshed renderer grid"
        )
    }
    ...
    private static func verifiesExplicitTerminalTabTitleIsLockedAgainstRuntimeMetadata() {
        let controller = makeController()

        guard let tabID = controller.openTerminalTab(title: "Automation Smoke"),
              let paneID = controller.selectedPane?.paneID
        else {
            fail("test setup must create a titled terminal tab")
        }

        let createdTab = controller.shellState.spaces.flatMap(\.tabs).first { $0.tabID == tabID }
        expect(
            createdTab?.isTitleUserLocked == true,
            "explicit terminal tab title must lock the tab title"
        )

        controller.updateTerminalMetadata(
            metadata(title: "runtime cwd shell title", cwd: "/repo/app"),
            for: paneID
        )

        let updatedTab = controller.shellState.spaces.flatMap(\.tabs).first { $0.tabID == tabID }
        expect(
            updatedTab?.title == "Automation Smoke",
            "runtime metadata must not overwrite an explicit terminal tab title"
        )
    }
    ...
    private static func verifiesAdvancedControlPlaneResizeEqualizeAndEvents() {
        ...
        expect(
            resizeResponse.terminalGridLayoutPolicy == .maxFit,
            "resize response must report the active terminal grid layout policy"
        )
        expect(
            resizeResponse.terminalGridLayoutEffects?.map(\.paneID) == ["pane_1", "pane_2"],
            "resize response must report terminal grid effects for affected panes"
        )
        expect(
            resizeResponse.terminalGridLayoutEffects?.first?.plannedGrid?.columns == 145,
            "resize response must distinguish terminal columns from host point width"
        )
        expect(
        ...
        expect(
            equalizeResponse.terminalGridLayoutPolicy == .maxFit,
            "equalize response must continue reporting the active renderer-derived grid policy"
        )
        expect(
            equalizeResponse.terminalGridLayoutEffects?.count == 2,
            "equalize response must report terminal grid effects for affected panes"
        )
        expect(
            controlEvents(controller).contains {
    ...
    private static func verifiesControlPlaneGridPolicyGuardForMixedContentSplits() {
        let fileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("MixedGridGuard-\(UUID().uuidString).md")
        do {
            try "## Mixed grid guard\n".write(to: fileURL, atomically: true, encoding: .utf8)
        } catch {
            fail("mixed grid guard setup must create a markdown file: \(error)")
        }
        defer { try? FileManager.default.removeItem(at: fileURL) }

        let controller = makeController()
        guard let tabID = controller.selectedTabID else {
            fail("mixed grid guard setup must expose a selected tab")
        }
        guard let markdownPaneID = controller.splitPane(
            paneID: "pane_1",
            placement: .right,
            ...
            fail("mixed grid guard setup must create a markdown split")
        }
        guard let splitNodeID = controller.selectedTab?.paneTree.splitNodes.first?.nodeID else {
            fail("mixed grid guard setup must create a split node")
        }
        projectTerminalGridDiagnostics(
            controller: controller,
            paneID: "pane_1",
        ...
        expect(resizeResponse.applied == true, "mixed resize_split must still apply")
        expect(
            resizeResponse.affectedPaneIDs == ["pane_1", markdownPaneID],
            "mixed resize response must include both terminal and markdown PaneSlots"
        )
        expect(
            resizeResponse.terminalGridLayoutPolicy == nil,
            "mixed resize must not advertise a terminal-only grid policy"
        )
        expect(
            resizeResponse.terminalGridLayoutEffects?.map(\.paneID) == ["pane_1"],
            "mixed resize may report only the terminal leaf grid diagnostics"
        )
        ...
        expect(equalizeResponse.applied == true, "mixed equalize_splits must still apply")
        expect(
            equalizeResponse.affectedPaneIDs == ["pane_1", markdownPaneID],
            "mixed equalize response must include both terminal and markdown PaneSlots"
        )
        expect(
            equalizeResponse.terminalGridLayoutPolicy == nil,
            "mixed equalize must not advertise split-fit terminal-only policy"
        )
        expect(
            equalizeResponse.terminalGridLayoutEffects?.map(\.paneID) == ["pane_1"],
            "mixed equalize may report terminal diagnostics without quantizing markdown content"
        )
        expect(
            controller.shellState.contentStateProjection().contentMounted(in: markdownPaneID)?.kind == .markdown,
            "mixed equalize guard must leave markdown content point-native"
        )
    }
    ...
    private static func verifiesControlPlaneResponsesExposeContentContainers() {
        ...
        controller.updateTerminalRuntime(
            TerminalHostRuntimeSnapshot(
                ...
                renderer: TerminalRendererSnapshot(
                    ...
                    phase: .surfaceReady,
                    summary: "surface ready",
                    detail: "Ghostty surface attached",
                    failureReason: nil,
                    recentEvents: []
                ),
                paneMetadata: TerminalPaneMetadataSnapshot(
                ...
                surfaceState: AlanTerminalSurfaceStateSnapshot(
                    ...
                    terminalGridDiagnostics: TerminalGridDiagnostics(
                        ...
                        canvasPoints: TerminalGridPointSize(width: 1240, height: 760),
                        backingPoints: TerminalGridPointSize(width: 2480, height: 1520),
                        cellPoints: TerminalGridPointSize(width: 8.5, height: 18.5),
                        layoutPolicy: .maxFit,
                        remainder: TerminalGridRemainder(columns: 0, rows: 0),
                        mismatchStatus: .converged
                    ),
                    lastUpdatedAt: Date(timeIntervalSince1970: 1_201)
                ),
                lastUpdatedAt: Date(timeIntervalSince1970: 1_202)
            )
        )
        ...
        let terminalContent = stateResponse.contents?.first { $0.kind == .terminal }
        expect(
            terminalContent?.rendererState.phase == "surface_ready",
            "control-plane state response must preserve terminal ContentInstance renderer state"
        )
        expect(
            terminalContent?.payload.terminal?.terminalGridDiagnostics?.plannedGrid?.columns == 145,
            "control-plane state response must expose terminal ContentInstance grid diagnostics"
        )
        expect(
    ...
    private static func projectTerminalGridDiagnostics(
        controller: ShellHostController,
        paneID: String,
        columns: Int,
        rows: Int,
        canvasWidth: Double = 1240,
        canvasHeight: Double = 760,
        cellWidth: Double = 8.55,
        cellHeight: Double = 18.5,
        policy: TerminalGridLayoutPolicy
    ) {
        let diagnostics = TerminalGridDiagnostics(
            plannedGrid: TerminalGridDimensions(columns: columns, rows: rows),
            rendererGrid: TerminalGridDimensions(columns: columns, rows: rows),
            ptyGrid: TerminalGridDimensions(columns: columns, rows: rows),
            canvasPoints: TerminalGridPointSize(width: canvasWidth, height: canvasHeight),
            backingPoints: TerminalGridPointSize(width: canvasWidth * 2, height: canvasHeight * 2),
            cellPoints: TerminalGridPointSize(width: cellWidth, height: cellHeight),
            layoutPolicy: policy,
            remainder: TerminalGridRemainder(widthPoints: 0.25, heightPoints: 1.5),
            mismatchStatus: .converged
        )
        controller.updateTerminalRuntime(
            TerminalHostRuntimeSnapshot(
                stage: .windowAttached,
                contentID: ShellContentInstance.terminalContentID(forPaneID: paneID),
                paneID: paneID,
                tabID: controller.selectedTabID,
                logicalSize: CGSize(width: canvasWidth, height: canvasHeight),
                backingSize: CGSize(width: canvasWidth * 2, height: canvasHeight * 2),
                displayName: "Studio Display",
                displayID: "display_1",
                attachedWindowTitle: "alan",
                isFocused: controller.selectedPane?.paneID == paneID,
                renderer: .placeholder,
                paneMetadata: .placeholder,
                surfaceState: AlanTerminalSurfaceStateSnapshot(
                    readiness: .ready,
                ...
                terminalGridDiagnostics: diagnostics,
                lastUpdatedAt: Date(timeIntervalSince1970: 1_101)
            )
        )
    }

    private static func encodedTerminalGridDiagnostics(
        _ context: ShellContextSnapshot
    ) -> [String: Any]? {
        let data = try! JSONEncoder().encode(context)
        let object = try! JSONSerialization.jsonObject(with: data) as? [String: Any]
        return object?["terminal_grid_diagnostics"] as? [String: Any]
    }
