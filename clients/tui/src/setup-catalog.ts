...
export function buildHostConfigContent(channel: InstallChannelId = "stable"): string {
  const descriptor = installChannelDescriptor(channel);
  const daemonUrl = new URL(descriptor.daemonUrl);
  const daemonPort = daemonUrl.port || (daemonUrl.protocol === "https:" ? "443" : "80");
  const setupBindAddress = `${
    daemonUrl.hostname === "localhost" ? "127.0.0.1" : daemonUrl.hostname
  }:${daemonPort}`;
  return `# alan Host Configuration
# Generated by alan init wizard

bind_address = "${setupBindAddress}"
