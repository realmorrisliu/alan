...
use super::{turn_support::tool_result_preview, virtual_tool::VirtualToolOutcome};

mod runtime_inputs;

pub(super) use runtime_inputs::AgentInteractionRuntime;

pub(super) async fn handle_request_confirmation<E, F>(
    runtime: AgentInteractionRuntime<'_>,
    ...
    F: std::future::Future<Output = ()>,
{
    ...
    if let Some(mut pending) = parse_confirmation_request(&tool_call.id, tool_arguments) {
        pending.details =
            append_skill_permission_hints(pending.details, runtime.machine.active_skills());
...
pub(super) async fn handle_request_user_input<E, F>(
    runtime: AgentInteractionRuntime<'_>,
    tool_call: &NormalizedToolCall,
    tool_arguments: &serde_json::Value,
    emit: &mut E,
    ...
    F: std::future::Future<Output = ()>,
{
    ...
    if let Some(request) = parse_structured_user_input_request(&tool_call.id, tool_arguments) {
        let request_id = runtime
            .agent_files
...
pub(super) async fn handle_update_plan<E, F>(
    runtime: AgentInteractionRuntime<'_>,
    tool_call: &NormalizedToolCall,
    tool_arguments: &serde_json::Value,
    emit: &mut E,
    ...
    F: std::future::Future<Output = ()>,
{
    ...
    match parse_plan_update(tool_arguments) {
        Some((explanation, items)) => {
            runtime.machine.set_plan_snapshot_at_message_count(
                explanation.clone(),
                items.clone(),
                runtime.machine.messages().len(),
            );
...
pub(super) fn parse_structured_user_input_request(
    tool_call_id: &str,
...
pub(super) fn parse_plan_status(raw: &str) -> Option<alan_agent_protocol::PlanItemStatus> {
    match raw {
...
pub(super) fn parse_plan_update(
    arguments: &serde_json::Value,
...
pub(super) fn request_confirmation_tool_definition() -> ToolDefinition {
    ToolDefinition {
...
pub(super) fn request_user_input_tool_definition() -> ToolDefinition {
    ToolDefinition {
