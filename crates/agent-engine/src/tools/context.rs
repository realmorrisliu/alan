...
impl ToolExecutionBinding {
    ...
    pub fn from_launch_context(
        ...
        let cwd = context
            .host_cwd()
            .context("Process cwd is not backed by an explicit Host Mount")?;
        Ok(Self {
            cwd,
            namespace_cwd: PathBuf::from(&context.cwd),
...
impl ToolContext {
    ...
    pub fn project_text(&self, text: &str) -> String {
        ...
        let mut mounts = self.host_mounts.iter().collect::<Vec<_>>();
        ...
        for grant in mounts {
            ...
            for candidate in candidates {
                projected =
                    projected.replace(candidate.to_string_lossy().as_ref(), &grant.namespace_path);
            }
        }
        projected
    }
