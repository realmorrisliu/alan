...
#[tokio::test]
async fn tool_process_rejects_a_revoked_cwd_instead_of_using_another_grant() {
    #[derive(Debug)]
    struct FallbackGrant;

    impl crate::tools::ToolExecutionAuthority for FallbackGrant {
        fn reconcile(
            &self,
            _pid: u64,
            mut binding: ToolExecutionBinding,
            ...
            binding.set_adapter(Arc::new(TestAdapter {
                namespace_cwd: PathBuf::from("/mnt/other"),
                cwd: PathBuf::from("/host/other"),
            }));
            Ok(binding)
        }
    }
