...
//! Seatbelt profile; Linux applies a Landlock filesystem ruleset in a
//! `pre_exec` hook (`apply_landlock`). Detection is conservative: it reports an
//! OS backend only when its tooling is present, otherwise the path-guard
//! fallback (which forbids unattended bash/network). Both backends confine
//! writes to the workspace + temp; Seatbelt additionally denies network
//! (Linux network control via seccomp/ABI-v4 nets is a follow-up).
...
pub fn seatbelt_profile(workspace_root: &Path) -> String {
...
#[cfg(target_os = "linux")]
pub fn apply_landlock(workspace_root: &Path) -> std::io::Result<()> {
    use landlock::{
    ...
    Ruleset::default()
        // Best-effort: on kernels without Landlock network support the net
        // handling degrades rather than erroring; `confines_network()` is the
        // authority the policy consults before treating network as contained.
        ...
        .map_err(to_io)?
        // No net rules added => all TCP bind/connect denied.
        .restrict_self()
        .map_err(to_io)?;
    Ok(())
}
...
mod tests {
    ...
    fn seatbelt_profile_confines_writes_and_denies_network() {
        let profile = seatbelt_profile(&PathBuf::from("/work/space"));
    ...
    fn seatbelt_enforces_workspace_write_boundary_on_macos() {
        ...
        let profile = seatbelt_profile(workspace.path());
    ...
    fn landlock_enforces_workspace_write_boundary_on_linux() {
        ...
        let run = |script: String| {
            ...
            unsafe {
                cmd.pre_exec(move || super::apply_landlock(&root));
            }
            cmd.status().unwrap()
        };
    ...
    fn landlock_confines_network_on_linux() {
        ...
        cmd.arg("-c").arg(probe);
        unsafe {
            cmd.pre_exec(move || super::apply_landlock(&workspace_path));
        }
