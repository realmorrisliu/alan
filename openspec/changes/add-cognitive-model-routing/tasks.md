...
- [ ] 2.1 Add capability-aware evaluation through the existing Connection boundary; verify typed success, no-match, malformed result, timeout and cancellation fixtures.
- [ ] 2.2 Compose deterministic/evaluation/generation transitions in Agent Machine; verify structured-only completion, wait/resume and bounded fallback.
- [ ] 2.3 Reuse governed effect dispatch and durable evidence; verify missing capability stays unavailable and Unknown effects are not replayed.
- [ ] 2.4 Verify candidate exposure and immutable package references; ensure model/profile secrets and runtime evidence never enter packages.
