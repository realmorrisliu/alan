...
- [x] 1.1 Replace the navigation group order with General, Terminal, Agent, and System.
- [x] 1.2 Add a group-section model that can assemble user-task sections from existing row IDs without rebuilding row contents.
...
- [x] 2.4 Add an Alan-only selector affordance at the top of Agent without showing Codex until Codex settings are supported.
- [x] 2.5 Render only the selected group in the main content area while preserving existing row controls and value labels.
- [x] 2.6 Preserve compact unavailable provider connection and skill catalog rows inside Agent when daemon or skill catalog data is unavailable.

## 3. Tests

- [x] 3.1 Update model tests for General, Terminal, Agent, and System group order.
- [x] 3.2 Add row membership tests proving Terminal Profiles and Managed Terminal Accounts stay in Terminal and do not appear under Agent provider connection rows.
- [x] 3.3 Add row membership tests proving provider connection, runtime defaults, skills, skill package path, and command line tool rows appear in Agent.
- [x] 3.4 Add row membership tests proving daemon endpoint, shell state, shell control, and diagnostics rows appear in System.
- [x] 3.5 Keep or update redaction tests so grouped visible text does not expose secrets, raw custom commands, or credential setting names.
- [x] 3.6 Keep or update tests for compact unavailable provider connection and skill catalog behavior.
- [x] 3.7 Keep or update existing Settings singleton and non-terminal content lifecycle tests.

## 4. Verification

- [x] 4.1 Run `bash clients/apple/scripts/test-shell-settings-surface.sh`.
