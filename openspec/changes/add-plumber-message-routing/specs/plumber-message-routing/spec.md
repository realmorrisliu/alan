...
### Requirement: Plumbfs routes typed messages by rule, sender decoupled
Alan OS SHALL provide `plumbfs`, a file server where a sender writes a typed
...
Alan OS SHALL append every plumb message to an observable log stream, and rule
...
- **THEN** they read the plumb message log stream and the rule files
...
### Requirement: Plumbing is a composition mechanism, not the control path
Alan OS SHALL treat plumbing as a way to compose actors, not as the primary
...
- **THEN** it plumbs to a human inbox port, and the rule that decides this is an
  inspectable file
...
### Requirement: Plumbfs is a user-space file server over aP
Alan OS SHALL implement `plumbfs` as a user-space file server over the aP
