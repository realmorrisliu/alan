...
- Define strict durability as a launch guarantee that a producing Rollout
  ...
  terminal-write guarantee. Completed outcomes are reconstructible whenever
  discovery finds a complete valid `process_exit`, even after an ambiguous
  append or durable-sync error; only a missing or torn terminal record remains
  unfinished or incomplete evidence.
