...
## 5. File-server contract crate (`alan-fs`)

- [ ] 5.0 Create the standalone `alan-fs` crate (ADR-0025 D2) with no
  alan-specific dependencies; `alan-kernel` depends on it.
- [ ] 5.1 Define the file-server trait over fids: `walk`, `open`, `read`,
  ...
  (D5, wire-shaped).
- [ ] 5.2 Define a byte/offset stream file kind with `read` that blocks until new
  bytes are available and supports resume from an offset (D8); no separate
  notification primitive.
- [ ] 5.3 Implement the in-process fast-path transport that dispatches contract
  calls without serialization, so high-rate streams pay no protocol cost.
- [ ] 5.4 Add a serialization round-trip test over the contract's wire shape to
  prove a dumb byte transport could carry every operation unchanged (guards the
  D5 discipline before any wire transport exists).
