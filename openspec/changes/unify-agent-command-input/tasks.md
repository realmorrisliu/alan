...
- [ ] 2.7 Project command results into shared evidence and bounded model input; verify shared-cwd-relative path projection for `pwd`, diagnostics and direct stdout, no raw Host root or `/mnt` alias, truncation, readable references, retention gaps, exit status and a later Agent question about the preceding command without an automatic summary call.
...
- [ ] 2.14 Align structured project read/edit/search path parameters with Host shell cwd and paths through existing Host adapters. Verify Agent edit → native read/git diff, native edit → Agent read, grant-relative and shared-cwd-relative Agent paths plus native absolute shell paths, pending-buffer/save failure, stale-content conflict, read-only grants, symlink containment and revocation; no mirror copies or shell-text rewriting.
