...
- [ ] 2.7 Project command results into shared evidence and bounded model input; verify shared-cwd-relative path projection for `pwd`, diagnostics and direct stdout within the active grant, no raw Host root or `/mnt` alias, truncation, readable references, retention gaps, exit status and a later Agent question about the preceding command without an automatic summary call.
