...
- **WHEN** a renderer host runs on a daemon-backed compatibility path during a
  migration period
- **THEN** that path is treated as a compatibility adapter
- **AND** the renderer host's architecture target remains direct reads from
  `/proc` and `/agent` plus `ctl` writes
